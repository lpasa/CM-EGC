...
class CMEGC(torch.nn.Module):
    def __init__(self,
        ...
        self.fc = torch.nn.Linear(n_pc, out_feats, bias=bias)
    ...
    def forward(self, graph, feat):
        ...
        with graph.local_scope():
            ...
            for i, k_feat in enumerate(feat_list):
                result += self.fc(head_input * (beta.pow(i) / factorial(i)))
...
class CMEGC_Tanh(CMEGC):
    def __init__(self,
                 ...
                 norm=None,
                 allow_zero_in_degree=False):
        super(CMEGC_Tanh, self).__init__(
            in_feats,
            out_feats,
            k,
            n_pc,
    ...
    def get_hyper_beta(self, graph, feat):
        ...
        with graph.local_scope():
            ...
            for i, k_feat in enumerate(feat_list):
                beta_list.append(beta.pow(i) / factorial(i))


        torch.cuda.empty_cache()
        return beta_list
    def init_from_mem_no_PCA(self,graph, feat,lambda_input_sizes):
        with graph.local_scope():
            if not self._allow_zero_in_degree:
                if (graph.in_degrees() == 0).any():
                    raise DGLError('There are 0-in-degree nodes in the graph, '
        ...
        for head in range(self._n_heads):
            ...
            lambda_input_size = lambda_input_sizes[head]

            if lambda_input_size - last_index <= 0:
                lambda_input_size = last_index + 1

            self.head_index.append((last_index, lambda_input_size + last_index))
            if self._n_layers_heads <= 1:

                self.heads_fun.append(
                    torch.nn.Linear(lambda_input_size, 1))
            else:
                self.heads_fun.append(
                    SimpleMultiLayerNN(lambda_input_size, [self._k] * self._n_layers_heads, 1,
                                       hidden_act_fun=torch.nn.Tanh()))
            last_index = lambda_input_size + last_index
            self.beta.append(torch.nn.Parameter(torch.FloatTensor(1)))

    def init_from_mem(self,graph, feat,lambda_input_sizes):
        with graph.local_scope():
            if not self._allow_zero_in_degree:
                if (graph.in_degrees() == 0).any():
                    raise DGLError('There are 0-in-degree nodes in the graph, '
            ...
            if self._cached_VS is None:
                ...
                for head in range(self._n_heads):
                    ...
                    if self._n_layers_heads <= 1:
                        ...
                        self.heads_fun.append(
                            torch.nn.Linear(lambda_input_size, 1).to(feat_list[0].device))
                    else:
                        self.heads_fun.append(
                            SimpleMultiLayerNN(lambda_input_size, [self._k] * self._n_layers_heads, 1,
                                               hidden_act_fun=torch.nn.Tanh()).to(feat_list[0].device))
...
class CMEGC_Tanh_inc_deep(CMEGC_Tanh):
        ...
        with graph.local_scope():
            ...
            if self._cached_VS is None:
                ...
                for head in range(self._n_heads):
                    ...
                    self.head_index.append((last_index, last_index+lambda_input_size))
                    if head == 0:
                        print("model head dim: ", lambda_input_size )
                        self.heads_fun.append(
                            torch.nn.Linear(lambda_input_size, 1).to(feat_list[0].device))
                    else:
                        print("model head dim: ", lambda_input_size)
                        self.heads_fun.append(
                            SimpleMultiLayerNN(lambda_input_size, [self._k] * (head), 1,
                                               hidden_act_fun=torch.nn.Tanh()).to(feat_list[0].device))
                    last_index = lambda_input_size+last_index
                    self.beta.append(torch.nn.Parameter(torch.FloatTensor(1)).to(feat_list[0].device))
                self._cached_head_input = torch.mm(V, torch.diag(S)).to(feat_list[0].device)
...
class CMEGC_Tanh_same_input(CMEGC_Tanh):
